...
import { drawEmptyLine, drawLine, updateLine, updatePoint } from "./helper";
import { Vector3 } from "@babylonjs/core/Maths/math.vector";
import { VertexBuffer } from "@babylonjs/core/Buffers/buffer";

import { Texture } from "@babylonjs/core/Materials/Textures/texture";
import { PBRMaterial } from "@babylonjs/core/Materials/PBR/pbrMaterial";
import { cloneVertexData } from "./utils";
...
export class Cloth {
    ...
    readonly initialVertexData: VertexData;
    ...
    protected readonly fixedIndices: number[] = [];
    ...
    constructor(vertexData: VertexData, scene: Scene) {
        this.mesh = new Mesh("carpet", scene);

        this.vertexData = vertexData;
        this.initialVertexData = cloneVertexData(vertexData);
        ...
        this.mesh.material = new PBRMaterial("carpet", scene);
        this.mesh.material.wireframe = true;

        this.createSprings();
    }

    protected createSprings() {
        ...
        /*for (let i = 0; i < indices.length; i += 3) {
            ...
            const structuralSpring1 = Spring.FromIndices(p1, p2, this.positions, this.STRUCTURAL_KS, this.STRUCTURAL_KD);
            const structuralSpring2 = Spring.FromIndices(p2, p3, this.positions, this.STRUCTURAL_KS, this.STRUCTURAL_KD);
            const structuralSpring3 = Spring.FromIndices(p1, p3, this.positions, this.STRUCTURAL_KS, this.STRUCTURAL_KD);

            structuralSpring1.helperMesh = drawEmptyLine(new Color3(1, 0, 0), this.mesh.getScene());
            structuralSpring2.helperMesh = drawEmptyLine(new Color3(0, 1, 0), this.mesh.getScene());
            structuralSpring3.helperMesh = drawEmptyLine(new Color3(1, 0, 1), this.mesh.getScene());

            this.springs.push(structuralSpring1);
            this.springs.push(structuralSpring2);
            this.springs.push(structuralSpring3);
        }*/
        ...
        for (let x = 0; x < this.nbSubdivisions; x++) {
            for (let y = 0; y < this.nbSubdivisions - 1; y++) {
                ...
                //horizontalSpring.helperMesh = drawEmptyLine(new Color3(1, 0, 0), this.mesh.getScene());
    ...
    private pruneSprings() {
        //TODO: remove all redundant springs (we generate too many)
    }
    ...
    reset() {
        this.vertexData = cloneVertexData(this.initialVertexData);
        ...
        for (let i = 0; i < this.positions.length; i++) {
            ...
            this.forces[i].copyFromFloats(0, 0, 0);
        }
    }
